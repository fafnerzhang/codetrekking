#!/usr/bin/env python3
"""
FIT File Processor - Implements FitnessFileProcessor abstract interface
"""
import fitparse
...
class FitFieldMapper:
    """FIT field mapper that handles all fields without unknown prefixes/suffixes"""
...
class FitDataValidator(DataValidator):
    """FIT data validator"""
...
class FitDataTransformer(DataTransformer):
...
class FitFileProcessor(FitnessFileProcessor):
    """FIT file processor implementation"""
    
    def __init__(self, storage, options: Optional[ProcessingOptions] = None):
        super().__init__(storage, options)
        self.validator = FitDataValidator()
        self.transformer = FitDataTransformer()
        self.field_mapper = FitFieldMapper()
    ...
    def process(self, source: Union[str, Path, IO], 
        ...
        try:
            ...
            if not self.validate_source(source):
                ...
                result.add_error("Invalid FIT file format")
                return result
            ...
            if isinstance(source, (str, Path)):
                ...
                if not Path(fit_file_path).exists():
                    result.status = ProcessingStatus.FAILED
                    result.add_error(f"FIT file not found: {fit_file_path}")
                    return result
            ...
            logger.info(f"✅ FIT file processed: {result.successful_records} successful, {result.failed_records} failed")
            
        except Exception as e:
            result.status = ProcessingStatus.FAILED
            result.add_error(f"FIT file processing failed: {e}")
            result.processing_time = time.time() - start_time
            logger.error(f"❌ FIT file processing failed: {e}")
    ...
    def validate_source(self, source: Union[str, Path, IO]) -> bool:
        ...
        try:
            ...
            logger.warning(f"FIT file validation failed: {e}")
            return False
    
    def extract_metadata(self, source: Union[str, Path, IO]) -> Dict[str, Any]:
        """Extract FIT file metadata"""
        metadata = {}
